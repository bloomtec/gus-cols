<?php
	...
	$uploadPath .= '/' . $_REQUEST['contentType'] . '/' . $_REQUEST['directory'];

	$verifyToken = md5('8206092654127895411132256' . $_REQUEST['timestamp']);

	if($_REQUEST['token'] != $verifyToken) {
		echo json_encode(array('name' => $fileName, 'size' => 0, 'status' => 'error', 'info' => 'Error de verificación. Intente de nuevo.'));
