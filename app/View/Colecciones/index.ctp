<div class="colecciones index">
	<h2><?php echo __('Colecciones'); ?></h2>
	<table cellpadding="0" cellspacing="0">
	<tr>
			<th><?php echo $this->Paginator->sort('id'); ?></th>
			<th><?php echo $this->Paginator->sort('usuario_id'); ?></th>
			<th><?php echo $this->Paginator->sort('grupo_id'); ?></th>
			<th><?php echo $this->Paginator->sort('nombre'); ?></th>
			<th><?php echo $this->Paginator->sort('es_auditable'); ?></th>
			<th><?php echo $this->Paginator->sort('acceso_anonimo'); ?></th>
			<th><?php echo $this->Paginator->sort('created'); ?></th>
			<th><?php echo $this->Paginator->sort('modified'); ?></th>
			<th class="actions"><?php echo __('Actions'); ?></th>
	</tr>
	<?php foreach ($colecciones as $coleccion): ?>
	<tr>
		<td><?php echo h($coleccion['Coleccion']['id']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($coleccion['Usuario']['documento'], array('controller' => 'usuarios', 'action' => 'view', $coleccion['Usuario']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($coleccion['Grupo']['nombre'], array('controller' => 'grupos', 'action' => 'view', $coleccion['Grupo']['id'])); ?>
		</td>
		<td><?php echo h($coleccion['Coleccion']['nombre']); ?>&nbsp;</td>
		<td><?php echo h($coleccion['Coleccion']['es_auditable']); ?>&nbsp;</td>
		<td><?php echo h($coleccion['Coleccion']['acceso_anonimo']); ?>&nbsp;</td>
		<td><?php echo h($coleccion['Coleccion']['created']); ?>&nbsp;</td>
		<td><?php echo h($coleccion['Coleccion']['modified']); ?>&nbsp;</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View'), array('action' => 'view', $coleccion['Coleccion']['id'])); ?>
			<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $coleccion['Coleccion']['id'])); ?>
			<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $coleccion['Coleccion']['id']), null, __('Are you sure you want to delete # %s?', $coleccion['Coleccion']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
	'format' => __('Page {:page} of {:pages}, showing {:current} records out of {:count} total, starting on record {:start}, ending on {:end}')
	));
	?>	</p>
	<div class="paging">
	<?php
		echo $this->Paginator->prev('< ' . __('previous'), array(), null, array('class' => 'prev disabled'));
		echo $this->Paginator->numbers(array('separator' => ''));
		echo $this->Paginator->next(__('next') . ' >', array(), null, array('class' => 'next disabled'));
	?>
	</div>
</div>
<div class="actions">
	<h3><?php echo __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('New Coleccion'), array('action' => 'add')); ?></li>
		<li><?php echo $this->Html->link(__('List Usuarios'), array('controller' => 'usuarios', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Usuario'), array('controller' => 'usuarios', 'action' => 'add')); ?> </li>
		<li><?php echo $this->Html->link(__('List Grupos'), array('controller' => 'grupos', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Grupo'), array('controller' => 'grupos', 'action' => 'add')); ?> </li>
	</ul>
</div>
